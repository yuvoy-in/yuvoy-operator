  ...
  return (
    <main className="bg-cream text-forest min-h-dvh">
      <div className="container-page max-w-2xl py-8">
        ...
        <p className="eyebrow text-terra-deep mt-6">Your account</p>
        <h1 className="font-display tracking-display mt-3 text-4xl leading-[1.05]">
          Payout details
        </h1>
        <p className="text-forest/70 mt-3 text-base">
          Changing where the money goes takes two days on purpose. Here is
          exactly what happens, and how to stop it.
        </p>
          ...
          {inFlight.length > 0 ? (
            ...
            */
            <p className="text-forest/80 mt-3 text-sm">
              There is already a change in progress. Stop it above before
              raising another — two at once would mean whichever is approved
              last silently wins.
            </p>
          ) : (
            <div className="mt-4">
        ...
        {history.length > 0 ? (
          <section className="mt-10" aria-labelledby="history">
            ...
            <ul className="border-cream-line mt-3 border-t">
              {history.map((r) => (
                <li
                  key={r.id}
                  className="border-cream-line flex items-baseline justify-between gap-4 border-b py-3 text-sm"
                >

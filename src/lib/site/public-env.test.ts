...
describe("secretPublicVars", () => {
  it("finds nothing in an ordinary environment", () => {
    expect(
      secretPublicVars({
        NEXT_PUBLIC_SITE_URL: "https://operators.yuvoy.in",
  ...
  it("names every public variable that arrived as the placeholder, sorted", () => {
    expect(
      secretPublicVars({
        NEXT_PUBLIC_TRAVELLER_APP_URL: SECRET_PLACEHOLDER,
        NEXT_PUBLIC_SITE_URL: ` ${SECRET_PLACEHOLDER}\n`,
        NEXT_PUBLIC_ALLOW_INDEXING: "false",
      }),
    ).toEqual(["NEXT_PUBLIC_SITE_URL", "NEXT_PUBLIC_TRAVELLER_APP_URL"]);
  });

  it("leaves a server-only Secret alone", () => {
    // Read when a request arrives, and by then Vercel supplies the value.
    expect(secretPublicVars({ OPERATOR_API_URL: SECRET_PLACEHOLDER })).toEqual(
      [],
    );
  });
...
describe("assertNoSecretPublicVars", () => {
  ...
  it("stops the build, naming each variable and the fix", () => {
    ...
    expect(run).toThrow(/NEXT_PUBLIC_SITE_URL, NEXT_PUBLIC_TRAVELLER_APP_URL:/);
...
describe("next.config.ts", () => {
  ...
  it("refuses to load when a public variable arrived as a Secret", async () => {
    vi.stubEnv("NEXT_PUBLIC_TRAVELLER_APP_URL", SECRET_PLACEHOLDER);

 ...
 *
 * It has happened twice. yuvoy-app lost three production deploys to the
 ...
 * analytics run, and not one event recorded. Here the quiet case is
 * `NEXT_PUBLIC_TRAVELLER_APP_URL`: `travellerAppOrigin()` falls back to the
 * production app without a word, so a preview would link the live site.
 *
 * So `next.config.ts` calls this before the config is built, and the build
 * stops with the variables to fix instead of shipping a portal that half
 * works. Only public variables are checked: a server-only Secret such as
 * `OPERATOR_API_URL` is read when a request arrives, and by then Vercel

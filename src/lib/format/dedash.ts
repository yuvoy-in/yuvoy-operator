 ...
 * dash in anything an operator reads) cannot reach another team's database,
 * so it is applied here, on the way in, rather than at each screen that
 * happens to render one of these fields.
 *
 * This changes PUNCTUATION ONLY and never a word, which is the contract
 * `sentence()` already works under: it capitalises the first letter and adds a
 * full stop, and leaves the wording exactly as the API chose it.
 *
